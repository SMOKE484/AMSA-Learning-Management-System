...
const makeStyles = (colors: BrandPalette) => StyleSheet.create({
  ...
  overviewLabel: { fontSize: 13, color: colors.textSecondary, fontWeight: '600', marginBottom: 4 },
  overviewValue: { fontSize: 52, fontWeight: '800', color: colors.textPrimary, letterSpacing: -1, marginBottom: 12 },
  progressTrack: { height: 6, backgroundColor: colors.border, borderRadius: 3, overflow: 'hidden', marginBottom: 12 },
  ...
  testCountText: { fontSize: 13, color: colors.textMuted },
  ...
  termChipText:{ fontSize: 13, fontWeight: '600', color: colors.textSecondary },
  ...
  subjectName:  { fontSize: 17, fontWeight: '700', color: colors.textPrimary, letterSpacing: -0.3 },
  ...
  scoreValue:   { fontSize: 40, fontWeight: '800', color: colors.textPrimary, letterSpacing: -1 },
  scoreTotal:   { fontSize: 18, color: colors.textSecondary, marginLeft: 4, marginBottom: 6 },
  miniTrack:    { height: 5, backgroundColor: colors.border, borderRadius: 3, overflow: 'hidden', marginBottom: 14 },
  miniFill:     { height: '100%', borderRadius: 3 },

  testsWrap:    { backgroundColor: colors.surfaceAlt, borderRadius: 14, overflow: 'hidden' },
  testRow:      { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 14 },
  testRowBorder:{ borderBottomWidth: 1, borderBottomColor: colors.border },
  testName:     { fontSize: 13, fontWeight: '600', color: colors.textPrimary },
  ...
  return (
    <View style={s.container}>
      ...
      >
        ...
        {/* ── TERM SELECTOR ─────────────────────────────────────────────── */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.termScroll} contentContainerStyle={s.termContent}>
          {terms.map(term => {
            const active = selectedTerm === term;
            return (
              <TouchableOpacity
                key={term}
                style={[s.termChip, active ? { backgroundColor: BRAND.red, borderColor: BRAND.red } : { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}
                ...
                <View style={s.subjectInner}>
                  ...
                  <View style={s.testsWrap}>
                    {sub.marks.slice(0, 3).map((test: Mark, idx: number) => (
                      <View key={idx} style={[s.testRow, idx < Math.min(sub.marks.length, 3) - 1 && s.testRowBorder]}>
                        <View style={{ flex: 1 }}>
